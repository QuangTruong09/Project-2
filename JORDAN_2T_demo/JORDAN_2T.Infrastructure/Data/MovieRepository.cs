...
namespace JORDAN_2T.Infrastructure.Data
{
    ...
    public class MovieRepository : Repository<Movie>, IMovieRepository
    {
        ...
        /// <returns></returns>
        public IEnumerable<Movie> GetAdminMovies(MovieStatus status)
        {
            var movieList = _dbSet.Where(p => p.Status == status);

            return movieList.OrderByDescending(p => p.Number);
        }
       ...
       public IEnumerable<Movie> GetInventoryMovies(MovieSortKey sortOrder,int pg)
        ...
        public IEnumerable<Movie> GetCategory(int Id)
        {
            IEnumerable<Movie> movies;
            movies = _dbSet.Where(p => p.CategoryId == Id)
            .Where(p=>p.Status== MovieStatus.Active)
            .Include(p=>p.Photos);
            return movies;
        }
        public IEnumerable<Movie> GetSubCategory(int Id)
        {
            IEnumerable<Movie> movies;
            movies = _dbSet.Where(p => p.SubCategoryId == Id)
            .Where(p=>p.Status== MovieStatus.Active)
            .Include(p=>p.Photos);
